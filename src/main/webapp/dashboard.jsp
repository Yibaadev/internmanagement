...
<body>
	...
	<div class="header">
		...
		<div>
			Bienvenue, <strong>${supervisor.login}</strong> <a href="logout">Déconnexion</a>
		</div>
	</div>
	...
	<div class="container">
		...
		<div class="card">
			...
			<c:if test="${not empty error}">
				<div
					style="background: #fdecea; color: #c0392b; padding: 10px; margin-bottom: 15px; border-radius: 5px;">
					${error}</div>
			</c:if>
			...
			<form action="interManagement" method="post">
				...
				<input type="text" name="name" placeholder="Nom"
					value="${internToEdit.name}" required> <input type="text"
					name="surname" placeholder="Prénom" value="${internToEdit.surname}"
					required> <input type="email" name="email"
					placeholder="Email" value="${internToEdit.email}" required>

				<input type="date" name="startDate"
					value="${internToEdit.startDate}" required> <input
					type="date" name="endDate" value="${internToEdit.endDate}" required>

				<input type="number" name="group" value="${internToEdit.group}"
					required>
				...
				<tbody>
					<c:forEach var="intern" items="${listeStagiaires}">
						<tr>
							...
							<td>${intern.group}</td>
							<td><a class="edit"
								href="interManagement?action=edit&id=${intern.id}"> Modifier
							</a> | <a class="delete"
